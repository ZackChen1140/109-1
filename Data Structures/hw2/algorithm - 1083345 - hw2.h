...
namespace MyNamespace
{
   ...
   {
       int parent = (hole - 1) / 2;
       while (pred(first[parent], val) && hole > top)
       {
   ...
   {
       auto top = hole;
       auto childL = hole * 2 + 1;
       auto childR = hole * 2 + 2;
       while (childR < bottom)
       {
           pred(first[childR], first[childL]) ? first[hole] = first[childL] : first[hole] = first[childR];
           pred(first[childR], first[childL]) ? hole = childL : hole = childR;
           childL = hole * 2 + 1;
           childR = hole * 2 + 2;
       }
       if (childL < bottom)
       {
           first[hole] = first[childL];
           hole = childL;
       }
