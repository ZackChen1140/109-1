...
struct HashVec
{
   ...
   void assignGrow( const size_type cells, const value_type val )
   {
       ...
       while (myData.myLast != myData.myEnd)
           *myData.myLast++ = val;
...
//    return traitsObj( keyVal ) % maxidx;
   ...
   size_type bucket_size( size_type n ) const
   {
       ...
       value_type bucketSize = 0;
       while (it != myVec.myData.myFirst[n * 2 + 1])
           ++it, ++bucketSize;

       return it == myList.end() ? 0 : ++bucketSize;
   }
   ...
   void erase( const key_type &keyVal )
   {
       ...
       if (*head == *(head + 1))
           *head = *(head + 1) = myList.end();
       else if (*head == it || *(head + 1) == it)
           *head == it ? ++ * head : -- * (head + 1);
   ...
   {
       value_type hash = bucket(keyVal);
       auto it = myVec.myData.myFirst[hash * 2];
       while (it != myVec.myData.myFirst[hash * 2 + 1] && *it != keyVal)
           ++it;
